...
def test_read_json_with_patch(mock_file, mock_json) -> list:
...
def test_read_json_file_not_found() -> list:
    """Тест при отсутствии файла .json"""
    result = read_json("non_existent_file.json")
    assert result == []


def test_read_json_invalid_json(tmp_path) -> list:
...
def test_read_json_empty_file(tmp_path) -> list:
